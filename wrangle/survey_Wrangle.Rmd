...
myFiles <- list.files(path = mripPath, pattern = "trip*", all.files = FALSE,
...
```{r}
myFiles <- list.files(path = mripPath, pattern = "catch*", all.files = FALSE,
           full.names = TRUE, recursive = FALSE,
           ignore.case = FALSE, include.dirs = FALSE, no.. = FALSE)
...
HAB_in <- read.csv(myFiles[1]) %>% select(c(SAMPLE_DATE,LOCATION,COUNT_))

#if there is more than 1 file, as is the case with multiple years and months, step through them all. 
if (length(myFiles) > 1) {
  for (k in 2:length(myFiles)){
    HAB_in <- rbind(HAB_in,read.csv(myFiles[[k]]) %>% select(c(SAMPLE_DATE,LOCATION,COUNT_))) 
  }
}

head(HAB_in)

#Aggregate brevis_count for all monitors within the Sarasota area.
#grouped by year, month.  
#note that as more monitors were added over time, this will cause 
#the total brevis count to increase over time.  
#Scaling by the number of monitors might be helpful here. 
#I'll try first without scaling.  
HAB <- HAB_in %>%
  ...
  group_by(year,month) %>%
  summarize(brevis_count = sum(COUNT_),mean_brevis_count = round(mean(COUNT_)),.groups = "drop") 
...
HAB_MRIP <- inner_join(trip_catch,HAB,
           ...
           select(c(year,month,common,MODE_FX,AREA_X,catch_count,brevis_count,mean_brevis_count)) 
...
HAB_MRIP_small <- HAB_MRIP %>%
                  group_by(year,month,brevis_count,mean_brevis_count) %>%
                  summarize(catch_count = sum(catch_count),.groups = "drop") %>%
                  #add previous month brevis count 
                  arrange(year,month) %>%
                  mutate(previous_month_brevis=lag(brevis_count,n=1L,order_by=year,month,default=0)) %>%
